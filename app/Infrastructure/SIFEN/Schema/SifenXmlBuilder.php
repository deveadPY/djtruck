...
class SifenXmlBuilder
{
    public function buildFactura(array $data): string
    {
        ...
        $totalGs = number_format((float)$venta->precio_venta_moneda, 0, '.', '');
...
<rDE xmlns="http://ekuatia.set.gov.py/sifen/xsd"
  ...
  <DE Id="{$cdc}">
    ...
    <gTotSub>
      ...
      <dIVA5>0</dIVA5>
      <dIVA10>{$this->calcIva10((float)$venta->precio_venta_moneda)}</dIVA10>
      <dTotIVA>{$this->calcIva10((float)$venta->precio_venta_moneda)}</dTotIVA>
...
XML;
    ...
    private function calcIva10(float $totalConIva): string
    {
        return number_format(round($totalConIva / 11, 0), 0, '.', '');
    }
}
