    ...
    <div class="card">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route('proveedores.update', $proveedor->id) }}"
                data-confirm="Confirmar actualización de proveedor">
                @csrf @method('PUT')
                <div class="form-grid">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="nombre_fantasia"
                            value="{{ old('nombre_fantasia', $proveedor->nombre_fantasia) }}" maxlength="200">
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <select name="pais" required>
                            ...
                            <option value="US" {{ old('pais', $proveedor->pais) == 'US' ? 'selected' : '' }}>Estados Unidos
                            </option>
                            <option value="UY" {{ old('pais', $proveedor->pais) == 'UY' ? 'selected' : '' }}>Uruguay</option>
                            <option value="AR" {{ old('pais', $proveedor->pais) == 'AR' ? 'selected' : '' }}>Argentina
                            </option>
                        </select>
                    </div>
                    <div class="form-group">
                        ...
                        <select name="tipo" required>
                            @foreach(['DISTRIBUIDOR' => 'Distribuidor / Repuestos', 'FABRICANTE' => 'Fabricante / Origen', 'SERVICIO' => 'Servicios Técnicos', 'OTRO' => 'Gastos Operativos / Generales'] as $v => $l)
                                <option value="{{ $v }}" {{ old('tipo', $proveedor->tipo) == $v ? 'selected' : '' }}>{{ $l }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select name="moneda_principal" required>
                            <option value="USD" {{ old('moneda_principal', $proveedor->moneda_principal) == 'USD' ? 'selected' : '' }}>USD</option>
                            <option value="PYG" {{ old('moneda_principal', $proveedor->moneda_principal) == 'PYG' ? 'selected' : '' }}>PYG</option>
                            <option value="BRL" {{ old('moneda_principal', $proveedor->moneda_principal) == 'BRL' ? 'selected' : '' }}>BRL</option>
                        </select>
                    </div>
                    <div class="form-group">
                        <label>Estado Activo</label>
                        <select name="activo" required>
                            <option value="1" {{ old('activo', $proveedor->activo) ? 'selected' : '' }}>Sí, Activo</option>
                            <option value="0" {{ !old('activo', $proveedor->activo) ? 'selected' : '' }}>No, Inactivo
                            </option>
                        </select>
                    </div>
                </div>
