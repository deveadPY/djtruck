...
@section('page-title', '✏️ Editar Repuesto')
...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            <h2>{{ $repuesto->codigo }} — {{ $repuesto->descripcion }}</h2>
        </div>
        <div class="card-body">
            <form method="POST" action="{{ route('repuestos.update', $repuesto->id) }}" data-confirm="Confirmar actualización de repuesto">
                @csrf @method('PUT')
                <div class="form-grid">
                    <div class="form-group" style="grid-column:1/-1">
                        <label>Descripción *</label>
                        <input type="text" name="descripcion" value="{{ old('descripcion', $repuesto->descripcion) }}"
                            required>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select name="unidad_medida" required>
                            @foreach(['UND' => 'Unidad', 'LTS' => 'Litros', 'KG' => 'Kilogramos', 'MT' => 'Metros', 'JGO' => 'Juego', 'PAR' => 'Par'] as $v => $l)
                                <option value="{{ $v }}" {{ old('unidad_medida', $repuesto->unidad_medida) == $v ? 'selected' : '' }}>
                                    {{ $l }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group">
                        ...
                        <input type="number" name="stock_actual" value="{{ old('stock_actual', $repuesto->stock_actual) }}"
                            step="0.001" min="0">
                    </div>
                    <div class="form-group">
                        <label>Stock mínimo</label>
                        <input type="number" name="stock_minimo" value="{{ old('stock_minimo', $repuesto->stock_minimo) }}"
                            step="0.001" min="0">
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" name="costo_promedio_moneda" value="" step="0.01" min="0">
                    </div>
                    <div class="form-group">
                        <label>Precio de venta original</label>
                        <input type="number" name="precio_venta_moneda" value="" step="0.01" min="0">
                    </div>
                    <div class="form-group">
                        <label>Costo promedio (USD) *</label>
                        <input type="number" name="costo_promedio_usd"
                            value="{{ old('costo_promedio_usd', $repuesto->costo_promedio_usd) }}" step="0.01" min="0"
                            required>
                    </div>
                    <div class="form-group">
                        <label>Precio de venta (USD)</label>
                        <input type="number" name="precio_venta_usd"
                            value="{{ old('precio_venta_usd', $repuesto->precio_venta_usd) }}" step="0.01" min="0">
                    </div>
                </div>
                <div
