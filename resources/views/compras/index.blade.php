                            ...
                            <td class="text-right !pr-6">
                                <div class="flex items-center justify-end gap-1 opacity-40 group-hover:opacity-100 transition-opacity">
                                    ...
                                    @can('repuestos.eliminar')
                                        <button type="button" 
                                            @click="deleteAction = '{{ route('compras.destroy', $c->id) }}'; showDeleteModal = true"
                                            class="p-2 rounded-xl hover:bg-red-500/10 text-red-400 border border-transparent transition-all" title="Anular Compra">
                                            <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
            ...
            <div class="relative overflow-hidden rounded-2xl border border-white/5 bg-surface2/40 backdrop-blur-md p-4 transition-all duration-300 active:scale-[0.98]">
                ...
                <div class="flex items-center gap-2 pt-4 border-t border-white/5">
                    ...
                    @can('repuestos.eliminar')
                        <button type="button" 
                            @click="deleteAction = '{{ route('compras.destroy', $c->id) }}'; showDeleteModal = true"
                            class="w-11 h-11 flex items-center justify-center rounded-xl bg-red-500/10 text-red-500 border border-red-500/20 active:bg-red-500/20 transition-all">
                            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" />
                            </svg>
                        </button>
